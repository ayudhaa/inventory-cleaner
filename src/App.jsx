...
import { 
  ...
  Plus, Menu, X, History, Loader2, TrendingUp, Search, RefreshCcw
} from 'lucide-react';
...
const App = () => {
  ...
  useEffect(() => {
    ...
    // 3. Fetch Transactions & Chart Data
    const unsubTx = onSnapshot(query(collection(db, "transactions"), orderBy("timestamp", "asc")), (snap) => {
  ...
  const handleTransaction = async (e) => {
    ...
    if (!product) return Notify.failure('Pilih produk terlebih dahulu');
    if (txData.type === 'out' && product.stock < qtyNum) {
      return Report.failure('Stok Kurang', 'Sisa stok tidak mencukupi untuk transaksi ini.', 'Ok');
    }
    ...
    try {
      ...
      Notify.failure('Gagal Sinkronisasi dengan Server'); 
      console.error(err);
    } finally { 
      setIsSyncing(false); 
    }
  ...
  const NavItem = ({ id, icon: Icon, label }) => (
    <button 
      ...
      className={`flex items-center gap-3 px-6 py-4 rounded-[1.4rem] w-full transition-all ${
        activeTab === id ? 'bg-stone-900 text-white shadow-xl' : 'text-stone-400 hover:bg-stone-50'
      }`}
    >
      <Icon size={18} /> 
  ...
  if (appLoading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-[#FDFCFB]">
      ...
      <p className="mt-8 text-[10px] font-black tracking-[0.3em] text-stone-400 animate-pulse">Initializing Terminal</p>
    </div>
  );

  return (
    <div className="min-h-screen bg-[#FDFCFB] flex flex-col md:flex-row text-stone-700 relative overflow-x-hidden">
      
      {/* GLOBAL SYNC LOADER */}
      {isSyncing && (
        <div className="fixed inset-0 bg-white/60 backdrop-blur-[2px] z-[200] flex items-center justify-center">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl border border-stone-50 flex flex-col items-center">
            ...
            <p className="text-[9px] font-black tracking-widest text-stone-900">Syncing Data...</p>
          </div>
        </div>
      )}

      {/* Sidebar Navigation */}
      <aside className={`fixed inset-y-0 left-0 z-[60] w-72 bg-white border-r border-stone-50 p-10 transform transition-transform duration-500 md:relative md:translate-x-0 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="mb-16">
      ...
      {/* Main Content */}
      <main className="flex-1 p-6 md:p-16 lg:p-20 overflow-y-auto">
        {activeTab === 'dashboard' && (
          <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in duration-700">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                ...
                <button onClick={() => openTxModal()} className="bg-stone-900 text-white px-10 py-5 rounded-2xl font-black text-[10px] tracking-widest shadow-2xl hover:bg-black transition-colors">
                  Record Transaction
                </button>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white p-10 rounded-[3rem] border border-stone-50 shadow-sm flex justify-between items-end">
                <div>
                  <p className="text-stone-300 text-[9px] font-black tracking-widest mb-2">Total Items</p>
              ...
              <div className="bg-stone-900 p-10 rounded-[3rem] text-white flex justify-between items-end shadow-2xl">
                <div>
                  <p className="text-stone-500 text-[9px] font-black tracking-widest mb-2">Stock Warning</p>
                  <div className="text-5xl font-serif italic">{products.filter(p => p.stock <= storeConfig.lowStockLimit).length}</div>
                </div>
                <TrendingUp className="text-stone-800" size={60} />
              </div>
            </div>

            <div className="bg-white p-10 rounded-[3.5rem] border border-stone-50 shadow-sm overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              ...
              <div className="relative w-full md:w-80">
                ...
                <input 
                  ...
                  className="w-full bg-white border border-stone-100 rounded-2xl py-4 pl-14 pr-6 text-xs outline-none focus:border-stone-900 transition-all shadow-sm" 
                  value={searchTerm} 
                    ...
                    <div key={p.id} className="bg-white p-8 rounded-[2.5rem] border border-stone-50 shadow-sm hover:shadow-xl transition-all flex flex-col justify-between group">
                        <div>
                          <div className="flex justify-between items-start">
                            <span className="text-[8px] font-black tracking-widest text-stone-400 bg-stone-50 px-3 py-1 rounded-lg">{p.category || 'General'}</span>
                        ...
                        <div className="mt-8">
                          <div className="flex justify-between items-end border-t border-stone-50 pt-6 mb-6">
                              <span className="text-[9px] font-black text-stone-300 tracking-tighter">Stok Saat Ini</span>
                          ...
                          <button onClick={() => openTxModal(p.id, 'out')} className="w-full bg-stone-900 text-white py-4 rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black tracking-widest hover:bg-black transition-all shadow-lg">
        ...
        {activeTab === 'transactions' && (
          <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-500">
            <header>
                ...
                <p className="text-[10px] font-black tracking-widest text-stone-300">Detailed Transaction History</p>
            </header>
                ...
                <div key={t.id} className="bg-white p-6 rounded-[2.2rem] border border-stone-50 flex items-center justify-between shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] font-black text-stone-300 tracking-widest">{t.dateText}</p>
                    </div>
                  </div>
      ...
      {showModalTx && (
        <div className="fixed inset-0 bg-stone-900/10 backdrop-blur-2xl z-[100] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-md rounded-[3.5rem] p-12 shadow-2xl relative border border-stone-50 animate-in zoom-in-95 duration-300">
            {!isSyncing && (
            ...
            <form onSubmit={handleTransaction} className="space-y-6">
              <div className="flex bg-stone-50 p-2 rounded-[2rem]">
                <button type="button" onClick={() => setTxData({...txData, type: 'out'})} className={`flex-1 py-4 rounded-[1.6rem] text-[10px] font-black tracking-widest transition-all ${txData.type === 'out' ? 'bg-stone-900 text-white shadow-lg' : 'text-stone-400'}`}>Keluar</button>
                <button type="button" onClick={() => setTxData({...txData, type: 'in'})} className={`flex-1 py-4 rounded-[1.6rem] text-[10px] font-black tracking-widest transition-all ${txData.type === 'in' ? 'bg-stone-900 text-white shadow-lg' : 'text-stone-400'}`}>Masuk</button>
              </div>

              <div className="space-y-2">
                  <label className="text-[9px] font-black tracking-widest text-stone-400 ml-4">Pilih Produk</label>
                  <select 
                    className="w-full bg-stone-50 rounded-2xl p-5 text-sm outline-none cursor-pointer border-none appearance-none" 
                    value={txData.productId} 
                    onChange={e => setTxData({...txData, productId: e.target.value})} 
                    required 
                    disabled={isSyncing}
                  >
              ...
              <div className="space-y-2">
                  <label className="text-[9px] font-black tracking-widest text-stone-400 ml-4">Jumlah Kuantitas</label>
                  <input 
                    type="number" 
                    className="w-full bg-stone-50 rounded-2xl p-5 text-sm outline-none" 
                    placeholder="Masukkan angka..." 
                    value={txData.qty} 
                    onChange={e => setTxData({...txData, qty: e.target.value})} 
                    required 
                    min="1" 
                    disabled={isSyncing}
                  />
              ...
              <button disabled={isSyncing} className="w-full bg-stone-900 text-white py-6 rounded-[2rem] font-bold text-[10px] tracking-widest flex items-center justify-center gap-3 hover:bg-black transition-all shadow-xl disabled:bg-stone-400">
                {isSyncing ? (
                    <>
